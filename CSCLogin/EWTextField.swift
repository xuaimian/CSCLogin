...
class EWTextField: UITextField {
    ...
    private let bottomLine : CALayer = {
        let line = CALayer()
        line.backgroundColor = UIColor.x333333.cgColor
        line.isHidden = false
        return line
    }()
   @objc public let label : UILabel = {
    ...
    ///   - frame: frame
    ///   - isSecure: 是否是密码格式
   @objc init(frame:CGRect,isSecure:Bool) {
   ...
   @objc public func changeLabel() {
    ...
    @objc public func changeLineBlueColor() {
       // self.bottomLine.isHidden = !self.bottomLine.isHidden
        
        self.bottomLine.backgroundColor = UIColor.x4FB0FF.cgColor
        
    }
    @objc public func changeLineGreyColor() {
        // self.bottomLine.isHidden = !self.bottomLine.isHidden
        
        self.bottomLine.backgroundColor = UIColor.x333333.cgColor
        
    }
    /// placeholder下移方法,当文字清空时调用
   @objc public func disChangeLabel() {
        UIView.animate(withDuration: 0.4) {
